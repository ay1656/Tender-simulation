...
class AntigenDemand:
    def __init__(self, antigen_data, simulation_duration):
        ...
        self.reorder_point = {year: self.doses_required*self.time_space*self.average_birth_rate for year in range(1, simulation_duration + 1)}
...
class Vaccine:
    def __init__(self, vaccine_name, simulation_duration):
        self.name = vaccine_name
        self.simulation_duration = simulation_duration
        self.vaccine_names = ["VaccineA", "VaccineB", "VaccineC", "VaccineD", "VaccineE", "VaccineF"]
    ...
    def calculate_provider_capabilities(self, year):
        ...
        for vaccine in self.vaccine_names:
...
antigen_demand_objs = [AntigenDemand(antigen_data, simulation_duration) for antigen_data in antigen_data_list]

# Create a list to hold instances of Vaccine for each vaccine
vaccine_objs = [Vaccine(vaccine_name, simulation_duration) for vaccine_name in vaccines_to_simulate]
...
def simulation(env, antigen_demand_objs, vaccine_objs, simulation_duration, committed_doses):
    ...
    while current_year <= simulation_duration:
        for antigen_obj in antigen_demand_objs:
            if current_year % antigen_obj.time_space == 0 and not antigen_obj.scheduled_tenders.get(current_year, False):
                # Call schedule_tender only when the time space condition is met and no tender is scheduled
                antigen_obj.schedule_tender(current_year)

                # Schedule the tender by updating the scheduled_tenders dictionary
                antigen_obj.scheduled_tenders[current_year] = True
