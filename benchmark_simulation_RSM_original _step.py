...
def main(num_replicates=int(input("Enter number of replicates: "))):
    ...
    file_paths = ['initial_conditions_0000000_3_1_1.csv','initial_conditions_0000000_3_1_105.csv', 'initial_conditions_0000000_3_1_110.csv','initial_conditions_0000000_3_6_1.csv', 'initial_conditions_0000000_3_6_105.csv','initial_conditions_0000000_3_6_110.csv', 'initial_conditions_0000000_3_10_1.csv','initial_conditions_0000000_3_10_105.csv', 'initial_conditions_0000000_3_10_110.csv','initial_conditions_0000000_4_1_1.csv','initial_conditions_0000000_4_1_105.csv','initial_conditions_0000000_4_1_110.csv','initial_conditions_0000000_4_6_1.csv','initial_conditions_0000000_4_6_105.csv','initial_conditions_0000000_4_6_110.csv', 'initial_conditions_0000000_4_10_1.csv','initial_conditions_0000000_4_10_105.csv','initial_conditions_0000000_4_10_110.csv', 'initial_conditions_0000000_5_1_1.csv', 'initial_conditions_0000000_5_1_105.csv', 'initial_conditions_0000000_5_1_110.csv', 'initial_conditions_0000000_5_10_1.csv', 'initial_conditions_0000000_5_10_105.csv', 'initial_conditions_0000000_5_10_110.csv','initial_conditions_0000000_5_6_1.csv', 'initial_conditions_0000000_5_6_105.csv', 'initial_conditions_0000000_5_6_110.csv']
    ...
    for replicate in range(num_replicates):
        ...
        for file_path in file_paths:
            ...
            final_results_df = pd.concat([final_results_df, pd.DataFrame(yearly_results)], ignore_index=True)
            ...
            print("Final Yearly Results DataFrame:")
            print(final_results_df)
            print("Objective Values DataFrame:")
            print(objective_df)
       
            # Save the final results to a CSV file   
            final_results_filename= f'final_results_{num_years}_{seed}_years.csv'
            objective_results_filename= f'all_objective_results_{num_years}_{seed}_years.csv'
...
def main():
    ...
    #model_directory = os.path.join(os.curdir, "/home/ay1656", "tender-scheduling")

    # Initialize AMPL
    ampl = AMPL()

    # Set solver
    ampl.set_option("solver", "gurboi")

    # List of .run files to execute
    experiments = ["Tenderschedule0","Tenderschedule1", "Tenderschedule001145", 'Tenderschedule001155', 'Tenderschedule01', 'Tenderschedule1', 'Tenderschedule2', 'Tenderschedule4', 'Tenderschedule5', 'Tenderschedule6', 'Tenderschedule7', 'Tenderschedule8', 'Tenderschedule9', 'Tenderschedule10', 'Tenderschedule11','Tenderschedule12', 'Tenderschedule13', 'Tenderschedule14']
    for experiment in experiments:
        try:
            print(f"experiment:", experiment)
            ampl.read(os.path.join(model_directory, experiment + ".run"))
        except:
            pass
...
# Assuming you have already run the main function and have the objective_df DataFrame
# Add a column to identify the combination number
replicates =int(input("Confirm replicates:"))
objective_df['Combination'] = (objective_df.index // replicates) + 1

# Define the factor levels
factor_a_levels = [1, 2, 3]
factor_b_levels = [1, 2, 3]
factor_c_levels = [1, 2, 3]

# Create a mapping of combination to factors
combinations = [(a, b, c) for a in factor_a_levels for b in factor_b_levels for c in factor_c_levels]
...
objective_df['FactorC'] = objective_df['Combination'].map(lambda x: factor_mapping[x][2])

# Group by 'Combination' and compute the mean of 'Objective Value'
averaged_df = objective_df.groupby(['FactorA', 'FactorB', 'FactorC'])['Objective Value'].mean().reset_index()

# Add squared terms
averaged_df['FactorA_squared'] = averaged_df['FactorA'] ** 2
averaged_df['FactorB_squared'] = averaged_df['FactorB'] ** 2
averaged_df['FactorC_squared'] = averaged_df['FactorC'] ** 2

# Print the averaged DataFrame
print("Averaged DataFrame:")
print(averaged_df)

# Assuming 'BlockFactor1' and 'BlockFactor2' should be categorical and added
# Example values for blocking factors
averaged_df['BlockFactor1'] = ['Block1'] * 9 + ['Block2'] * 9 + ['Block3'] * 9
averaged_df['BlockFactor2'] = ['BlockA'] * 9 + ['BlockB'] * 9 + ['BlockC'] * 9
...
# Verify new columns
print("Updated DataFrame with Blocking Factors:")
...
# Step 1: Fit the second-order model including quadratic and interaction terms
second_order_formula = ('Objective_Value ~ FactorA + FactorB + FactorC + '
                        'I(FactorA**2) + I(FactorB**2) + I(FactorC**2) + '
                        'FactorA:FactorB + FactorA:FactorC + FactorB:FactorC + '
                        'C(BlockFactor1) + C(BlockFactor2)')
...
linear_terms = ['FactorA', 'FactorB', 'FactorC']
...
def response_surface_plot(model, x1_name, x2_name, x3_name):
    ...
    x3 = np.linspace(averaged_df[x3_name].min(), averaged_df[x3_name].max(), 100)
    ...
    df['FactorC_squared'] = df[x3_name] ** 2
    df['FactorA:FactorB'] = df[x1_name] * df[x2_name]
    df['FactorA:FactorC'] = df[x1_name] * df[x3_name]
    df['FactorB:FactorC'] = df[x2_name] * df[x3_name]

    # Ensure all required columns are present
    required_columns = [
        x1_name, x2_name, x3_name,
        'FactorA_squared', 'FactorB_squared', 'FactorC_squared',
        'FactorA:FactorB', 'FactorA:FactorC', 'FactorB:FactorC',
    ...
    ax.plot_surface(x1[:, :, 0], x2[:, :, 0], y[:, :, 0], cmap='viridis')
...
response_surface_plot(second_order_model, 'FactorA', 'FactorB', 'FactorC')