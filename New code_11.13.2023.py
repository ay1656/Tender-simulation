...
class VaccineProcurementSimulation:
    ...
    def tender_scheduling(self, year, production_lead_time):
        while True:
            for antigen in self.antigens:
    ...
    def run_simulation(self, num_years, production_lead_time):
        for antigen in self.antigens:
            # Start SimPy processes for each antigen
            self.env.process(self.process_demand(antigen))
            self.env.process(self.tender_scheduling(antigen, production_lead_time))
...
def main():
    ...
    simulation = VaccineProcurementSimulation(env, initial_conditions)

    # Run the simulation with a production lead time of 2 time units
    simulation.run_simulation(num_years=15, production_lead_time=2)
