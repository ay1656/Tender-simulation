...
# Define the run_simulation function here
def run_simulation(seed, num_years, production_lead_time, file_paths):
    try:
        all_yearly_results = []
        ...
        return final_results_df, objective_df, seed
...
def main(num_replicates=int(input("Enter number of replicates: "))):
    ...
    file_paths = ['conditions_1111111._5_1_b.csv', 'conditions_1111111._5_5_b.csv', 'conditions_1111111._5_8_b.csv', 'conditions_1111111._6_1_b.csv']
    ...
    # Initialize dataframes to accumulate results
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        ...
        for replicate in range(num_replicates):
            seed = replicate + 1
            futures.append(executor.submit(run_simulation, seed, num_years, production_lead_time, file_paths))
        
        for future in concurrent.futures.as_completed(futures):
            final_results, objective_results, seed = future.result()
            
            # Accumulate the results
            final_results_df = pd.concat([final_results_df, final_results])
            objective_df = pd.concat([objective_df, objective_results])
            
            # Save the results to CSV
            final_results_filename = f'final_results_{num_years}_{seed}_years_b.csv'
            objective_results_filename = f'all_objective_results_{num_years}_{seed}_years_b.csv'
